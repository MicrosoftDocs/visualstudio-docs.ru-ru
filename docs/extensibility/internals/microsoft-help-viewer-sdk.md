...
|vs_logo_wh.gif|Используется для ссылок на высококонтрастный логотип, как определено в теге Branding.xml \<LogoFileNameHC> .  Для продуктов Visual Studio имя логотипа — vs_logo_wh.gif.||
...
2. В этом примере мы будем использовать файлы MSHC для хранения разделов.  MSHC — это ZIP-файл с расширением файла, измененный с .zip на. MSHC.
