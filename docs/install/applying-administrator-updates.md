...
* **Servicing baseline stickiness** (Закрепление базового плана обслуживания). Как описано выше, обновления администратора, являющиеся обновлениями компонентов, приводят к переходу установки Visual Studio на более позднюю дополнительную версию продукта. Но в ряде случаев команды разработчиков предпочитают использовать определенный стабильный и безопасный базовый план обслуживания, и им нужно контролировать, когда их клиенты переходят на более актуальную дополнительную версию. Чтобы настроить клиентский компьютер так, чтобы он продолжал использовать базовый план обслуживания и игнорировал нежелательные обновления компонентов администратора, отправляемые ему, вам необходимо создать значение данных Reg_SZ **BaselineStickinessVersions2019** и установить его для строки, представляющей допустимые базовые планы, к которым клиентский компьютер может подключаться и которые может использовать.  Строка может содержать последовательность версий базового плана обслуживания, разделенных запятыми, например **16.4.0,16.7.0**. В строку может быть добавлено любое количество версий базового плана обслуживания, а также слово **All**, которое является сокращением для обозначения всех поддерживаемых базовых планов обслуживания. 

     Если значение реестра `BaselineStickinessVersions2019` имеет неправильный формат, то установка всех обновлений компонентов на компьютер будет заблокирована. Кроме того, обратите внимание на [поддерживаемые временные рамки для обновлений компонентов Visual Studio](https://docs.microsoft.com/visualstudio/productinfo/vs-servicing-vs). Несмотря на то что технически возможно применять обновления компонентов, срок действия которых истек, мы не рекомендуем это делать, потому что они не будут поддерживаться и, следовательно, станут потенциально небезопасны.
