...
### <a name="confirm-editor-attaching-is-enabled-or-code-optimization-on-startup-is-set-to-debug"></a>Параметр Confirm `Editor Attaching` включен или `Code Optimization On Startup` установлен в значение `Debug`
...
Дополнительные сведения см. в [документации по настройкам Unity](https://docs.unity3d.com/Manual/Preferences.html).
...
Очень важно помнить, что Visual Studio сохраняет "несовместимое" состояние в параметрах проекта и не пытается перезагрузить проект, пока не будет явно использован `Reload Project` . Поэтому после каждого шага устранения неполадок убедитесь, что вы пытаетесь повторно открыть решение, и попробуйте щелкнуть правой кнопкой мыши все несовместимые проекты и выбрать `Reload Project` .

1. Убедитесь, что Visual Studio задана в качестве внешнего редактора скриптов в Unity с помощью `Edit / Preferences / External Tools` .
...
6. Попробуйте очистить кэш MEF, как описано [здесь](#visual-studio-crashes).
7. Попробуйте переустановить Visual Studio (используя рабочую нагрузку "игра" или "Unity" только для запуска).
...
Никогда не работайте с файлами проектов непосредственно из обработчика ресурсов или иного средства. Если вам действительно необходимо совершить операции с файлом проекта, мы предоставляем для этого интерфейс API. См. раздел [Проблемы со ссылками на сборку](#assembly-reference-or-project-property-issues).
...
При использовании новой среды выполнения Unity в зависимости от версии Unity также требуются пакеты целевой платформы .NET версии 4,6 или 4.7.1. Для быстрой установки (изменения установки, отдельных компонентов, категории .NET) можно использовать установщик Visual Studio. Выберите все 4. x нацеливание пакетов.

## <a name="assembly-reference-or-project-property-issues"></a>Проблемы со ссылкой на сборку или свойством проекта

Если ваш проект довольно сложен с точки зрения ссылок или требуется лучше контролировать этот этап создания, вы можете использовать наш [API](../extensibility/customize-project-files-created-by-vstu.md) для работы с созданным содержимым проекта или решения. Вы также можете использовать [файлы ответов](https://docs.unity3d.com/Manual/PlatformDependentCompilation.html) в своем проекте Unity и обрабатывать их.

В случае последних версий Visual Studio и Unity лучшим подходом будет использование пользовательского `Directory.Build.props` файла вместе с созданными проектами. После этого вы сможете участвовать в структуре проекта, не мешая процессу создания. Дополнительные сведения см. [здесь](https://docs.microsoft.com/visualstudio/msbuild/customize-your-build#directorybuildprops-and-directorybuildtargets).
...
Убедитесь, что используемый вами скрипт правильно загружен и используется в текущей сцене Unity. Выйдите из Visual Studio и Unity и удалите все созданные файлы ( \* . csproj, \* . sln), `.vs` папку и всю папку библиотеки. Дополнительные сведения об отладке C# можно найти на [веб-сайте](https://docs.unity3d.com/Manual/ManagedCodeDebugging.html)Unity.
...
## <a name="issues-with-intellisense-or-code-coloration"></a>Проблемы с IntelliSense или разметка кода

Попробуйте обновить Visual Studio до последней версии. Выполните те же действия по устранению неполадок, что и для [несовместимых проектов](#incompatible-project-in-visual-studio).
